...
class CityList:
    ...
    def __init__(self):
        ...
        self.__button_mc = Button(self.__window, text = "Moscow")
                                  #command = a.CityData.get_moscow)
        self.__button_mc.grid(row=2, column = 1)

        

        self.__button_me = Button(self.__window, text = "Ciudad_de_México")
                                  #command = a.CityData.get_mexico)
        self.__button_me.grid(row=2, column = 2)

        
        self.__button_is = Button(self.__window, text = "Istanbul")
                                  #command = a.CityData.get_istanbul)
        self.__button_is.grid(row=3, column = 0)

       
        self.__button_kr = Button(self.__window, text = "Karachi")
                                  #command = a.CityData.get_karchi)
        self.__button_kr.grid(row=3, column = 1)

        self.__button_jk = Button(self.__window, text = "Jakarta")
                                  #command = a.CityData.get_jakarta)
        self.__button_jk.grid(row=3, column = 2)

        self.__button_sh = Button(self.__window, text = "Shanghai")
                                  #command = a.CityData.get_shanghai)
        self.__button_sh.grid(row=4, column = 0)

        self.__button_sp = Button(self.__window, text = "São_Paulo")
                                  #command = a.CityData.get_sp)
        self.__button_sp.grid(row=4, column = 1)

        self.__button_kr = Button(self.__window, text = "Seoul")
                                  #command = a.CityData.get_seoul)
        self.__button_kr.grid(row=4, column = 2)

        self.__button_mb = Button(self.__window, text = "Mumbai")
                                  #command = a.CityData.get_mumbai)
